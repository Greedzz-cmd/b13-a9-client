    ...
    startTransition(async () => {
      try {
        const { error } = await authClient.signUp.email({
          ...
          image: formData.profileURL || null,
        });
  ...
  function handleGoogle() {
    startTransition(async () => {
      try {
        ...
        if (error) {
          toast.danger(
            error.message || "Google sign-up is not configured yet.",
          );
        }
      } catch (error) {
        toast.danger(error.message || "Google sign-up is not configured yet.");
      }
    });
  }
          ...
          <Form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              validate={(value) => {
                ...
                try {
                  ...
                  if (
                    ...
                    hostname.includes("encrypted-tbn")
                  ) {
                    return "Please use a direct image link, not a Google Search image URL";
                  }
                  if (!/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(value)) {
                    return "URL must point directly to an image (.jpg, .png, .webp, etc.)";
                  }
                  return null;
                } catch {
            ...
            >
              ...
              <Description className="mt-1 text-xs text-gray-400">
                Direct link to an image file — leave blank to use your initials
              </Description>
              <FieldError className="mt-1 text-xs text-red-500" />
            </TextField>
